...
class Current_AttendanceScreen_TA extends StatefulWidget {
  ...
  @override
  _Current_AttendanceScreen_TAState createState() =>
      _Current_AttendanceScreen_TAState();
}

class _Current_AttendanceScreen_TAState
    extends State<Current_AttendanceScreen_TA> {
  String totalStudent = '';
  String _search = '';
  String? _classtime;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData is Map<String, dynamic> &&
          ...
          int id = jsonData['attendance_list']['id'];
          setState(() {
            ListId = id;
          });
  ...
  void _updateAttendanceStatus(int index, String status) {
    setState(() {
      ...
      if (status == '1') {
        if (!selectedStudentIds.contains(AllStudentList[index].admnNo)) {
          selectedStudentIds.add(AllStudentList[index].admnNo);
        }
      } else {
        selectedStudentIds.remove(AllStudentList[index].admnNo);
      }
    });
  }
  ...
  void _selectAll() {
    setState(() {
      for (var student in AllStudentList) {
        student.status = '1';
        if (!selectedStudentIds.contains(student.admnNo)) {
          selectedStudentIds.add(student.admnNo);
        }
      }
      isallselected = false;
    });
  ...
  void _onAbsent(int index) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text("Mark Absent"),
        content: Text(
            "Are you sure you want to mark ${AllStudentList[index].admnNo} as absent?"),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> _AbsentAttendanceUpdate(String classpriod) async {
    ...
    try {
      ...
      final Uri uri = isNEP
          ...
          : Uri.parse(
              '${ApiConstants.baseUrl}UpdateAttendanceStatus_for_allStudent'); // If isNEP is false
